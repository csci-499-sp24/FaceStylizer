...
function FileUpload() {
  ...
  const passImagesToMlUi = () => {
    // Get the array of image URLs
    const imageUrls = Object.keys(files).map((url) => URL.createObjectURL(files[url]));
    return imageUrls;
  };
  ...
  return (
    <div className="mt-0 bg-white h-screen w-screen sm:px-8 md:px-16 sm:py-8">
      <main className="container mx-auto max-w-screen-lg h-full">
        {showUploadButton && ( // Conditionally render the FileUpload component
          <article
            ...
            <section className="h-full overflow-auto p-8 w-full h-full flex flex-col">
              ...
              <ul id="gallery" className="flex flex-1 flex-wrap -m-1">
                {Object.keys(files).map((url) => (
                  <li key={url} className="mb-20 block p-1 w-1/2 sm:w-1/3 md:w-1/4 lg:w-1/6 xl:w-1/8 h-24">
...
{showMlUi && <MlUi images={passImagesToMlUi()} />} {/* Conditionally render the MlUi component */}
